#!/usr/bin/env python3
"""
iBus Serial Monitor - Simple real-time RC channel monitor
Usage: python ibus_serial.py COM3
"""

import serial
import sys
import time
from protocols.ibus_decoder import IBUSDecoder
...
def display(ch, frames, rate, errors):
    ...
    p(f"  Ch13-14:  {ch[12]:4d}  {ch[13]:4d}  ----  ----")
    p("  (iBus supports max 14 channels)")
    p("")
    p("────────────────────────────────────────────────────────────────────────────")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python ibus_serial.py <PORT>")
        print("Example: python ibus_serial.py COM3")
        sys.exit(1)
